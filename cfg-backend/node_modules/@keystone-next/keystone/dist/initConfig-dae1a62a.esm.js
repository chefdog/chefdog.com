...
import { Q as QueryMode, f as fieldType, o as orderDirectionEnum } from './sqlite-335dc173.esm.js';
import _objectWithoutProperties from '@babel/runtime/helpers/esm/objectWithoutProperties';
import { g as graphqlBoundToKeystoneContext, J as JSON$1 } from './graphql-ts-schema-e8d59289.esm.js';
import { GraphQLString, executeSync, parse, GraphQLUnionType, GraphQLScalarType, GraphQLNonNull, GraphQLSchema, GraphQLObjectType, assertScalarType, assertEnumType, validate, execute, GraphQLList, print, graphql as graphql$1 } from 'graphql';
import { h as humanize, u as userInputError, e as extensionError, a as accessReturnError, b as accessDeniedError, v as validationFailureError, r as resolverError } from './graphql-errors-b69877ed.esm.js';
...
import { s as staticAdminMetaQuery } from './admin-meta-graphql-433469b3.esm.js';
...
import { r as resolveUniqueWhereInput, i as isRejected, a as isFulfilled, m as mapUniqueWhereToWhere, b as resolveWhereInput, c as runWithPrisma, g as getOperationAccess, d as getAccessFilters, e as checkFilterOrderAccess, f as getDBFieldKeyForFieldOnMultiField, p as promiseAllRejectWithAllErrors, h as findOne, j as findMany, k as count, l as getVariablesForGraphQLField, n as executeGraphQLFieldToRootVal, o as initialiseLists } from './types-for-lists-20b6587f.esm.js';
import { field, object } from '@graphql-ts/schema/api-with-context';
import { arg, nonNull, list, inputObject, Int, ID } from '@graphql-ts/schema/api-without-context';
...
function resolveRelateToManyForCreateInput(nestedMutationState, context, foreignList, target) {
  return async value => {
    if (!Array.isArray(value.connect) && !Array.isArray(value.create)) {
      throw userInputError(`You must provide at least one field in to-many relationship inputs but none were provided at ${target}`);
...
function resolveRelateToManyForUpdateInput(nestedMutationState, context, foreignList, target) {
  return async value => {
    ...
    if (value.set && value.disconnect) {
      throw userInputError(`The set and disconnect fields cannot both be provided to to-many relationship inputs but both were provided at ${target}`);
    } // Perform queries for the connections
...
function resolveRelateToOneForCreateInput(nestedMutationState, context, foreignList, target) {
  return async value => {
    ...
    if (numOfKeys !== 1) {
      throw userInputError(`Nested to-one mutations must provide exactly one field if they're provided but ${target} did not`);
    }
...
export { createSystem as c, initConfig as i, serializePathForImport as s, writeAdminFiles as w };
