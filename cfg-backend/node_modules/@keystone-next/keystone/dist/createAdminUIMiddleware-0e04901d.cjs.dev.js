...
var createApolloServer = require('./createApolloServer-7af27d7d.cjs.dev.js');
var url = require('url');
...
const createExpressServer = async (config, graphQLSchema, createContext) => {
  ...
  if ((_config$server3 = config.server) !== null && _config$server3 !== void 0 && _config$server3.extendExpressApp) {
    ...
    const createRequestContext = async (req, res) => createContext({
      sessionContext: config.session ? await session_dist_keystone.createSessionContext(config.session, req, res, createContext) : undefined,
      req
    });
