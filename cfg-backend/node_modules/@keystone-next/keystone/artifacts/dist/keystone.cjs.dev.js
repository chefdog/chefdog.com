...
var typesForLists = require('../../dist/types-for-lists-6ce03bcd.cjs.dev.js');
require('prompts');
...
function printField(fieldPath, field, datasourceName, lists) {
  if (field.kind === 'scalar') {
    ...
    return `${fieldPath} ${field.scalar}${modifiers[field.mode]}${updatedAt}${nativeType}${defaultValue}${index}`;
  }
  ...
  if (field.kind === 'multi') {
    return Object.entries(field.fields).map(([subField, field]) => printField(typesForLists.getDBFieldKeyForFieldOnMultiField(fieldPath, subField), field, datasourceName, lists)).join('\n');
  }
...
function printPrismaSchema(lists, provider, clientDir, prismaPreviewFeatures) {
  let prismaFlags = '';
...
async function getCommittedArtifacts(graphQLSchema, config) {
  ...
  const prismaSchema = printPrismaSchema(lists, config.db.provider, 'node_modules/.prisma/client', config.db.prismaPreviewFeatures);
  return {
