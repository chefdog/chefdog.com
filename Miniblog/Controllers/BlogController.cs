...
namespace Miniblog.Models
{
    public class BlogController : Controller
    {
        ...
        public async Task<IActionResult> Index([FromRoute]int page = 0)
        {
            ...
            ViewData["SubTitle"] = _settings.Value.SubTitle;
        ...
        public async Task<IActionResult> UpdatePost(Post post)
        {
            ...
            if (post.PreviewFile!=null)
            {
                post.PreviewImageUrl = await Upload(post);
            }
        ...
        private async Task<string> Upload(Post post)
        {
            ...
            {
                await post.PreviewFile.CopyToAsync(memoryStream);
                // Upload the file if less than 2 MB
                if (memoryStream.Length < 2097152)
                {
                    result = await _blog.SaveFile(memoryStream.ToArray(), post.PreviewFile.FileName);
                }
                else
                {
